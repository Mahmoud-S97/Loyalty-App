...
const MainButton = ({ testID, children, title, icon, image, className, textClassName, imageClassName, iconSize, iconColor, disabled, onPress }: MainButtonProps) => {
    ...
    return (
        <TouchableOpacity testID={testID ?? 'MainButton:Button'} style={shadowStyle(scheme)} activeOpacity={0.7} disabled={disabled} onPress={onPress} className={cn('flex-row w-full h-[50px] justify-center items-center bg-neutral-100 rounded-full', combinedClasses)}>
