...
const NotificationCard = ({
  item: { id, title, icon, body, createdAt },
  index,
  className,
  onPress
  ...
  const translateYAnimation = useRef(
    new Animated.Value(animatedIds.has(id) ? 0 : 20)
  ).current;
  ...
  return (
    <Animated.View
      testID='NotificationCard:AnimatedView'
      style={[
        {
      ...
      className={cn(
        'w-full min-h-100 px-4 py-6 border-b border-neutral-500 dark:border-neutral-800 bg-neutral-50 dark:bg-secondary',
        className
      )}
    >
      <TouchableOpacity
        ...
        onPress={onPress}
      >
        <View className='w-full flex flex-row gap-4 items-start justify-start'>
          <View className='flex-1/2 items-center justify-center'>
            <View className='w-20 h-20'>
              <Image
                testID='NotificationCard:Image'
                source={icon}
                resizeMode='contain'
