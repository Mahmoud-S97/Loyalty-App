  ...
  return (
    <ScrollingView className='bg-neutral-300'>
      <ContainerView className='items-start px-0'>
        ...
        <AppText withTranslation={false} numberOfLines={2} className='font-semibold text-xl mt-6 text-center self-center px-4'>Mahmoud Al-Saleh - محمود الصالح</AppText>
        ...
        {ACCOUNT_DETAILS.map((item: any, index: number) => (
          <View key={index} className='flex-1'>
            <AppText className='mt-10 mb-4 text-lg font-medium px-4'>{item.heading}</AppText>
