      ...
      <ScrollingView>
        ...
        <ContainerView className='pb-20'>
          <View className='flex-column w-full p-2 justify-center items-center my-7'>
            ...
            <AppText className='text-sm text-center font-bold text-neutral-800 dark:text-neutral-500 mt-4 tracking-wide'>auth.signUp_entries_with_welcoming_msg</AppText>
          </View>
          <View className='flex w-full p-2' >
            <MainInputField placeholder='example@gmail.com' icon='email' onChangeText={setEmailHandler} />
            <MainInputField placeholder='auth.password' icon='lock' className='my-7' secureTextEntry={!showPassword} isPasswordField={true} toggleShowPassword={toggleShowPassword} onChangeText={setPasswordHandler} />
            <MainInputField placeholder='auth.confirmPassword' icon='lock' className='mb-7' secureTextEntry={!showPassword} isPasswordField={true} toggleShowPassword={toggleShowPassword} onChangeText={setConfirmPasswordHandler} />
            <MainButton testID='SignUpScreen:SignUpBtn' className='bg-primary dark:bg-brand-800' title='auth.signUp' onPress={() => router.navigate('/home')} />
            <View className='flex-row justify-between items-center my-5'>
              <View className='w-[44%] h-[1px] bg-neutral-500' />
              <AppText className='w-[12%] text-center uppercase' weight='bold'>prepositions.or</AppText>
              <View className='w-[44%] h-[1px] bg-neutral-500' />
            </View>
            ...
            <TouchableOpacity testID='SignUpScreen:LoginBtn' activeOpacity={0.7} className='flex-row items-center justify-center mt-8' onPress={() => router.dismissTo('/login')}>
