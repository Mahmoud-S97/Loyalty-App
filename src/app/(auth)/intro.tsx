...
const IntroScreen = () => {
  return (
    <ScrollingView>
      <ContainerView>
        <View
          testID='IntroScreen:HeroSection'
          className='flex w-full justify-center items-center my-16 px-4'
        >
          <Image
            testID='IntroScreen:LogoImage'
            source={LOCAL_IMAGES.LOGO_TRANS}
            className='w-[120px] h-[120px] border-[1px] border-neutral-500 dark:border-neutral-800 rounded-lg'
          />
          <AppText className='text-2xl text-center mt-8' weight='bold'>
            welcome
          </AppText>
        </View>
        <View
          testID='IntroScreen:NavigationSection'
          className='flex-column w-full items-center justify-center p-2'
        >
          <MainButton
            testID='IntroScreen:LoginBtn'
            className='bg-primary dark:bg-brand-800'
            title='auth.login'
            onPress={() => router.replace('/login')}
          />
          <MainButton
            testID='IntroScreen:SignUpBtn'
            className='bg-neutral-700 mt-8'
            title='auth.signUp'
